...
- **Two ways to start**: **Recommended Practice** (3, 5, or 10 cues, default 5 — a deterministic list built from existing local evidence: a cue currently marked Misheard/Known but Not Heard/Connected or Reduced Speech, incorrect quiz evidence, recurring diagnosis history, and little-or-no shadowing practice as an amplifying signal only, never a qualifying one on its own — "little or no shadowing practice" counts both Intensive Practice's explicit shadowing and a learner's own explicit "Mark Shadowed" action from a completed Quick Practice cue, added together, never as a hidden score; every recommended cue shows its own transparent reason(s), and ties are broken only by genuine evidence timestamps, never an invented "recency" claim; when too little evidence exists, the list safely falls back to material order with no invented reasons) or **Selected Cues** (one cue, a continuous range, or an explicit multi-cue selection — order preserved exactly as picked).
...
- **A concise, read-only completion summary**: cues completed, Understood/Partly Understood/Missed counts, diagnoses created, explicit shadowing actions, and which cues are worth revisiting (a Missed result or at least one diagnosis) — never an effective-time, pronunciation, ability, difficulty, or improvement score. Recording activity during the run is not summarized as a count, since a recording has no ownership link back to the run that created it — it remains available through the Replay & Shadow step itself and through Learning History.
