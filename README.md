...
src/listentrace/
  ...
  ui/
    ...
    windows/          # MainWindow (material library), ImportDialog, PlayerWindow
                       # (with integrated transcript workspace), SettingsDialog (categorized
                       # Playback/Label Colors tabs, replacing the earlier standalone
                       # LabelColorDialog), MaterialLoopSettingsDialog,
...
- The packaged Windows build (installer and portable zip, see `packaging/README.md`) is unsigned — Product Owner approved for v1.0, not a release blocker; Windows SmartScreen may warn on first run depending on download/reputation context, though the Milestone 15.2 clean-machine install did not itself encounter a SmartScreen block. Install/launch/uninstall has been validated both on the development machine (Phase C1) and on a genuinely clean Windows 11 VM with no preinstalled Python (Milestone 15.2). Code signing, auto-update, and macOS/Linux packaging are not addressed in v1.0.
