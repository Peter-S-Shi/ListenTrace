...
- Canonical text-selection representation: `domain/services/text_range.py` defines offsets as zero-based, end-exclusive Python string (code-point) positions into the immutable cue `text`. This is deliberately chosen to match Qt's `QTextCursor.selectionStart()/selectionEnd()` for all Basic-Multilingual-Plane text, keeping the UI layer's only job "read the cursor, pass the two integers through" — no Qt-specific type ever reaches persistence. Documented limitation: characters requiring UTF-16 surrogate pairs are unverified.
...
- `heard_as` is attached per-row, not per-save-action: when a save creates multiple labels together (e.g. `keyword` + `misheard`), only the `misheard` row receives the typed `heard_as` value; other rows store `NULL`. Editing an annotation (`update_annotation`) only changes `heard_as`/`note` — changing the label or range is modeled as delete-and-recreate, not update, to avoid re-deriving uniqueness/validation for a mutated key.
