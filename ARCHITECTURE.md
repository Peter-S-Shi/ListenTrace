...
## Post-M10 Release Boundary

- Milestone 10 (Quick Practice Mode) is the final planned user-feature milestone for the first release; feature freeze takes effect once it is accepted, per `ROADMAP.md`.
- Milestone 11's optional assisted-service features (speech recognition, pronunciation feedback, translation, subtitle/question generation, pluggable external-service providers) are deferred beyond v1.0 — no assisted-service (network, AI, or pluggable-provider) architecture is required to ship v1.0.
- Packaging (`ROADMAP.md`'s Post-M10 Phase A) must preserve the local-data boundaries already established across Milestones 1-9: the database, recordings, and application-data locations, and the existing rule that original media/subtitle files are referenced in place and never copied, modified, or deleted by the application. Phase A has since decided a packaging technology and validated this boundary holds — see below.
...
- **No code signing, auto-update, or CI-driven build in this phase.** These are explicitly out of scope for a packaging *spike* (see `packaging/README.md`'s "Explicitly not addressed") and remain open items for Phase B/D.

## Architecture Decisions Resolved in Post-M10 Phase B (Release Hardening)

Phase B's job (`ROADMAP.md`) was corrective work against eleven specific failure categories. Two were found to already be solid, requiring no code change; five needed a real fix, confirmed by direct reproduction rather than assumption; the remainder were verified already correct and are recorded here so the investigation itself is not lost.
