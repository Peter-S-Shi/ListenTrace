...
- **Recommendation reads only pre-existing Milestone 4-7 evidence, never Quick Practice's own history** (`infrastructure/db/quick_practice_repository.py`'s `list_annotation_labels_by_cue`/`list_diagnosis_counts_by_cue`/`list_incorrect_quiz_evidence_by_cue`/`list_shadowing_stats_by_cue`). This avoids a circular "recommendations feeding on themselves" dependency and keeps the reason set anchored to evidence a learner would already recognize from the rest of the app.
