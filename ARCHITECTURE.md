...
- **Per-user install by default (`PrivilegesRequired=lowest`), not a mandatory machine-wide install.** Avoids a UAC prompt for an unsigned indie app with no admin-only resource requirements, while `PrivilegesRequiredOverridesAllowed=dialog` still lets a learner opt into a machine-wide install. Validated: the compiled installer ran silently with no elevation prompt and installed under `%LOCALAPPDATA%\Programs\ListenTrace`.
